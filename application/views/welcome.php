<div class="container-fluid mt-4">
	<!-- MULAI KONTEN DISINI -->

	<center>
		<h1>BeautyNeeds</h1>
	</center>

	<div class="container mt-4">
		<div class="row justify-content-between">	
			<div class="card text-white bg-info mb-3 shadow-lg" style="max-width: 21rem; border: none;">
				<!-- <div class="card-header">Header</div> -->
				<div class="card-body">
					<div class="row">
						<div class="col-4 my-auto">
							<i class="fas fa-fw fa-shipping-fast fa-4x"></i>
						</div>
						<div class="col">
							<h5 class="card-title">Gratis Ongkir</h5>
							<p class="card-text">Tenang saja, semua pengiriman produk kami tanpa biaya ongkir!</p>
						</div>
					</div>
				</div>
			</div>
			<div class="card text-white bg-info mb-3 shadow-lg" style="max-width: 21rem; border: none;">
				<!-- <div class="card-header">Header</div> -->
				<div class="card-body">
					<div class="row">
						<div class="col-4 my-auto">
							<i class="fas fa-fw fa-headset fa-4x"></i>
						</div>
						<div class="col">
							<h5 class="card-title">Support 24 Jam</h5>
							<p class="card-text">Sales kami akan selalu siap melayani Anda secara online 24 jam</p>
						</div>
					</div>
				</div>
			</div>
			<div class="card text-white bg-info mb-3 shadow-lg" style="max-width: 21rem; border: none;">
				<!-- <div class="card-header">Header</div> -->
				<div class="card-body">
					<div class="row">
						<div class="col-4 my-auto">
							<i class="fas fa-fw fa-handshake fa-4x"></i>
						</div>
						<div class="col">
							<h5 class="card-title">Cash On Delivery</h5>
							<p class="card-text">Pembayaran dapat dilakukan lebih mudah dengan Cash On Delivery</p>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>

	<!-- AKHIR KONTEN -->
</div>
